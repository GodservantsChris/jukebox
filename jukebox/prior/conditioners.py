...
class Conditioner(nn.Module):
    def __init__(self, input_shape, bins, down_t, stride_t, out_width, init_scale, zero_out, res_scale, **block_kwargs):
        emsgContext = f"conditioners.py.Conditioner.__init__()"
        emsgOperation = f""
        try:        
            super().__init__()
    ...
    def forward(self, x, x_cond=None):
        emsgContext = f"conditioners.py.Conditioner.forward()"
        emsgOperation = f""
        try:
            N = x.shape[0]
...
class SimpleEmbedding(nn.Module):
    def __init__(self, bins, out_width, init_scale):
        emsgContext = f"conditioners.py.SimpleEmbedding.__init__()"
        emsgOperation = f""
        try:        
            super().__init__()
            self.bins = bins
    ...
    def forward(self, y):
        emsgContext = f"conditioners.py.SimpleEmbedding.forward()"
        emsgOperation = f""
        try: 
...
class RangeEmbedding(nn.Module):
    ...
    def __init__(self, n_time, bins, range, out_width, init_scale, clamp=False):
        emsgContext = f"conditioners.py.RangeEmbedding.__init__()"
        emsgOperation = f""
        try:        
            super().__init__()
            self.n_time = n_time
    ...
    def forward(self, pos_start, pos_end=None):
        emsgContext = f"conditioners.py.RangeEmbedding.forward()"
        emsgOperation = f""
        try:        
...
class LabelConditioner(nn.Module):
    def __init__(self, y_bins, t_bins, sr, min_duration, max_duration, n_time, out_width, init_scale, max_bow_genre_size, include_time_signal):
        emsgContext = f"conditioners.py.LabelConditioner.__init__()"
        emsgOperation = f""
        try:        
            super().__init__()
            self.n_time = n_time
