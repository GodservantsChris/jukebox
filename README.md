...
``` 
python jukebox/sample.py --model=1b_lyrics --backend_to_run=gloo --name=/content/jukebox_outputs/sample_1b/ --levels=3 --sample_length_in_seconds=20 --total_sample_length_in_seconds=180 --sr=44100 --n_samples=3 --max_batch_size=16 --hop_fraction=0.5,0.5,0.125 --artist="Willie Nelson" --genre="Country" --lyrics="'''All dressed up to go dreaming Now don't tell me I'm wrong And what a night to go dreaming Mind, if I tag along? If I say, I love you, I want you to know It's not just because there's moonlight, although Moonlight becomes you, moonlight becomes you so'''"
```
